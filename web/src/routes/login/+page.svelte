...
<main class="mx-auto flex min-h-screen max-w-sm flex-col justify-center gap-6 p-4">
	<div class="text-center">
		...
		<p class="opacity-70">gira em torno do que importa</p>
	</div>

	<form class="flex flex-col gap-3" onsubmit={enviar}>
		<label class="flex flex-col gap-1">
			<span class="text-sm font-medium">E-mail</span>
			<input class="rounded border p-2" type="email" bind:value={email} required />
		</label>
		<label class="flex flex-col gap-1">
			<span class="text-sm font-medium">Senha</span>
			<input class="rounded border p-2" type="password" bind:value={senha} required />
		</label>
	...
	<p class="text-center text-sm">
		Não tem conta? <a class="underline" href="/cadastro">Criar conta</a>
	</p>
</main>
